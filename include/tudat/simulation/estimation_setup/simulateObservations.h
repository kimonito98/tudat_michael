        ...
        const SystemOfBodies& bodies )
{
    ...
    // Wire the per-leg LightTimeCalculator map into the calculator so that any deferred
    // `light_time_correction_components` dependent variables can be resolved.
    if( observationsToSimulate->getDependentVariableCalculator( ) != nullptr )
    {
        observationsToSimulate->getDependentVariableCalculator( )->setLegLightTimeCalculators(
