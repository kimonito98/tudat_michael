...
bool tryParseFixedWidthInt(
        const std::string& line, const unsigned int startIndex, const unsigned int length, int& output )
{
    return tryParseInt( getFixedWidthField( line, startIndex, length ), output );
}

bool tryParseFixedWidthDouble(
        const std::string& line, const unsigned int startIndex, const unsigned int length, double& output )
{
...
void addSatelliteIdToStateMap(
        const std::string& rawSatelliteId, Sp3FileContents& fileContents, std::vector< std::string >& knownSatelliteIds )
{
    ...
    for( const std::string& satelliteId : knownSatelliteIds )
    {
        ...
        if( flagIt == currentRecordFlags.end( ) || !flagIt->second.hasPosition )
        {
            throw std::runtime_error(
                    "Error when reading SP3 file: missing position record for satellite " + satelliteId + " at epoch " +
        ...
        {
            throw std::runtime_error(
                    "Error when reading SP3 file: missing velocity record for satellite " + satelliteId + " at epoch " +
                    std::to_string( currentTime ) + " in " + fileName );
        }
        ...
        {
            throw std::runtime_error(
                    "Error when reading SP3 file: missing state container for satellite " + satelliteId + " at epoch " +
                    std::to_string( currentTime ) + " in " + fileName );
        }
...
std::shared_ptr< Sp3FileContents > readSp3File( const std::string& fileName, const double referenceJulianDay )
{
    ...
    while( std::getline( stream, currentLine ) )
    {
        ...
        if( firstCharacter == '*' )
        {
            ...
            if( hasCurrentEpoch )
            {
                ...
                {
                    throw std::runtime_error(
                            "Error when reading SP3 file: epochs are not strictly increasing in " + fileName );
                }
        ...
        if( firstCharacter == 'P' || firstCharacter == 'V' )
        {
            ...
            {
                throw std::runtime_error(
                        "Error when reading SP3 file: encountered state record before first epoch in " + fileName );
            }
            ...
            const bool readSucceeded = tryParseFixedWidthDouble( currentLine, 4, 14, x ) &&
                                       tryParseFixedWidthDouble( currentLine, 18, 14, y ) &&
                                       tryParseFixedWidthDouble( currentLine, 32, 14, z );
            ...
            if( firstCharacter == 'P' )
            {
                ...
                {
                    throw std::runtime_error(
                            "Error when reading SP3 file: duplicate position record for satellite " + satelliteId + " in " +
                            fileName );
                }
            ...
            else
            {
                ...
                {
                    throw std::runtime_error(
                            "Error when reading SP3 file: duplicate velocity record for satellite " + satelliteId + " in " +
                            fileName );
                }
    ...
    if( fileContents->declaredNumberOfEpochs > 0 && parsedEpochCount != fileContents->declaredNumberOfEpochs )
    {
        throw std::runtime_error( "Error when reading SP3 file: parsed epoch count (" + std::to_string( parsedEpochCount ) +
                                  ") does not match declared epoch count (" +
                                  std::to_string( fileContents->declaredNumberOfEpochs ) + ") in " + fileName );
    }
